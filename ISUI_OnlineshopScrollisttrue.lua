...
function Onlinetest:doDrawItem(y, item, alt)
    ...
    self:drawTextureScaledAspect(itemtex_tex, 300,(y)+itemPadY-5, 32, 32, 1, 1, 1, 1)
    local itemtex_name = itemtex_item:getDisplayName()
    -- self:drawText(item_type, 15, (y)+itemPadY, 1.0, 1.0, 1.0, 1.0, self.font);
    self:drawText(itemtex_name.." : "..tostring(item.item.quantity), 15, (y)+itemPadY+2, 1.0, 1.0, 1.0, 1.0, UIFont.Medium);
    self:drawText("$"..item.item.price, 410, (y)+itemPadY+2, 1, 1, 0, 1, UIFont.Large);
    self:drawText(getText("IGUI_maijia").." : "..tostring(item.item.username), 580, (y)+itemPadY+2, 1.0, 1.0, 1.0, 1.0, UIFont.Medium);

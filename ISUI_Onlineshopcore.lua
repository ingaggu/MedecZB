...
function ISOnlineshopcore:createChildren()
    ...
    self.moneylabel = ISLabel:new(x, y+margin+370, FONT_HGT, "money", 1, 1, 0, 1.0, UIFont.Small, true);
...
function ISOnlineshopcore:prerender()
    ...
    self.moneylabel:setName("ID : "..self.character_:getUsername().."\n"..getText("IGUI_yuea").."$"..tostring(Onlineshopinfo.currentmoney).."\n"..getText("IGUI_yishangjia")..tostring(Onlineshopinfo.currentupitem).."/10")
...
function ISOnlineshopcore:onClick(_button)
    ...
    if _button.customData =="sell" then
        if Onlineshopinfo.currentshop.index == "basesysshop" then
			...
			-- Add this code to limit the input length to 5 digits
        ...
        elseif Onlineshopinfo.currentshop.index == "playerinv" then
			...
			-- Add this code to limit the input length of entry2 to 5 digits
			modal.entry2.onTextChange = function(entry)
				local text = entry:getText()
				if text:len() >= 2 then
					entry:setText(text:sub(1, 1))
				end
			end
